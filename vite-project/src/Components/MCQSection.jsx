...
export default function MCQSection() {
    ...
    const handleOptionChange = (question, selectedOption) => {
        setUserAnswers(prevAnswer => {
            const updatedAnswers = prevAnswer.filter(ans => ans.question !== question);
            return [...updatedAnswers, { question, userAnswer: selectedOption }];
        });
    };
    ...
    const handleSubmission = async() => {
        ...
        userAnswers.forEach(userAns => {
            const correctAns = correctAnswers.find(ans => ans.question === userAns.question);
            ...
            if(userAnswers.length===0 || userAnswers.length > 0){
